 ...
 *
 * Pricing verified against 2026 Stockholm market:
 *   AMK Transport, Lillebil, Svelands Försäkring, Swerock price lists.
 */
...
// ═══════════════════════════════════════════════════════════════════════════════
// 2026 STOCKHOLM MARKET RATES
// Sources: AMK Transport, Lillebil, Svelands, Swerock 2026 price lists
// ═══════════════════════════════════════════════════════════════════════════════

const DIESEL_PRICE = 21.50;  // kr/L, B7 2026

// Fuel consumption per km (l/km) — verified 2026 Stockholm fleet
const VEHICLE_FUEL = {
  'KEM-01': 0.30,  // kranbil 3-axel: 3.0 l/mil (range 2.5–3.5)
  'KEM-02': 0.36,  // lastväxlare 3-axel: 3.6 l/mil
  'KEM-03': 0.34,  // flakbil 3-axel <14t: 3.4 l/mil
  'KEM-04': 0.38,  // tippbil tung 9 000 kg: 3.8 l/mil (range 3.1–4.2)
  'KEM-05': 0.50,  // dragbil med kapellsläp: 5.0 l/mil (range 4–6)
  'KEM-06': 0.35,  // containerbil 3-axel: 3.5 l/mil
};
...
// Hourly rates (timpris, ex moms) — verified 2026:
//   Kranbil 3-axel: 1 067–1 150 kr/h  → 1 100 kr/h (midpoint)
//   Treaxlig lastbil: 1 029 kr/h
//   3-axlad <14 ton: 856 kr/h
//   Tung lastbil 9 000 kg: 767 kr/h
//
// Crane truck specs (verified 2026):
//   3-axel 30tm: lyft 13–14 ton, 12m sidonådd. Nypris ~5 000 000 kr.
//   4-axel 50tm: lyft 10–13 ton, 15–17m sidonådd. Nypris ~6 500 000 kr.
const FLEET = [
  {
    ext_id:'KEM-01', reg:'ABC 123', namn:'Volvo FH 540 Kranbil 30tm',
    lasttyp:'kranbil', typ:'kranbil', max_last_kg:24000, volym_m3:null,
    lez_godkand:1, euro_klass:6,
    timkostnad_sek:1100, priskm_sek:26, startavgift_sek:1650,
    forbrukning_l_per_km:0.30,
    beskrivning:'3-axel 30tm kranbil. Max lyft 13–14 ton, 12m sidonådd. Framkörning 1,5h. Euro 6. ADR klass 3.',
  },
  {
    ext_id:'KEM-02', reg:'DEF 456', namn:'Scania R 500 Lastväxlare',
    lasttyp:'lastväxlare', typ:'lastväxlare', max_last_kg:20000, volym_m3:36,
    lez_godkand:1, euro_klass:6,
    timkostnad_sek:1029, priskm_sek:26, startavgift_sek:1029,
    forbrukning_l_per_km:0.36,
    beskrivning:'3-axel lastväxlare med flakbyte. Container och skrothämtning. Euro 6.',
  },
  {
    ext_id:'KEM-03', reg:'GHI 789', namn:'Volvo FM 460 Flakbil',
    lasttyp:'flakbil', typ:'flakbil', max_last_kg:14000, volym_m3:42,
    lez_godkand:1, euro_klass:6,
    timkostnad_sek:856, priskm_sek:26, startavgift_sek:856,
    forbrukning_l_per_km:0.34,
    beskrivning:'3-axel flakbil <14 ton med bakgavellyft. Allmän godstransport. Euro 6.',
  },
  {
    ext_id:'KEM-04', reg:'JKL 012', namn:'Scania G 410 Tippbil',
    lasttyp:'tippbil', typ:'tippbil', max_last_kg:20000, volym_m3:null,
    lez_godkand:0, euro_klass:5,
    timkostnad_sek:767, priskm_sek:26, startavgift_sek:767,
    forbrukning_l_per_km:0.38,
    beskrivning:'Tippbil för massor och schakt. 9 000 kg driftvikt. Euro 5 — ej LEZ-godkänd, detour krävs till innerstad.',
  },
  {
    ext_id:'KEM-05', reg:'MNO 345', namn:'Mercedes Actros med kapellsläp',
    lasttyp:'dragbil', typ:'dragbil', max_last_kg:22000, volym_m3:82,
    lez_godkand:1, euro_klass:6,
    timkostnad_sek:1029, priskm_sek:32, startavgift_sek:1544,
    forbrukning_l_per_km:0.50,
    beskrivning:'Dragbil med kapellsläp 82 m³. Lång distans, täckt gods. 315 kr/mil (31,5 kr/km) vid körning >80 km. Euro 6.',
  },
  {
    ext_id:'KEM-06', reg:'PQR 678', namn:'MAN TGX Containerbil',
    lasttyp:'containerbil', typ:'containerbil', max_last_kg:20000, volym_m3:null,
    lez_godkand:1, euro_klass:6,
    timkostnad_sek:1029, priskm_sek:26, startavgift_sek:1029,
    forbrukning_l_per_km:0.35,
    beskrivning:'3-axel containerbil, 20/40 fot ISO-container. Euro 6.',
  },
];
...
// Cargo types paired with best-fit vehicle. rate = 2026 verified timpris (kr/h, ex moms).
const CARGO = [
  { typ:'Kranlyft',             veh:'KEM-01', tim:6.0, rate:1100 },
  { typ:'Grävmaskin transport', veh:'KEM-04', tim:4.5, rate:767  },
  { typ:'Prefab-element',       veh:'KEM-05', tim:5.0, rate:1029 },
  { typ:'Containertransport',   veh:'KEM-06', tim:3.5, rate:1029 },
  { typ:'Betongleverans',       veh:'KEM-03', tim:2.5, rate:856  },
  { typ:'Asfalt och beläggning',veh:'KEM-04', tim:3.0, rate:767  },
  { typ:'Jordmassor',           veh:'KEM-04', tim:3.5, rate:767  },
  { typ:'Byggmaterial',         veh:'KEM-03', tim:2.5, rate:856  },
  { typ:'Maskintransport',      veh:'KEM-01', tim:4.0, rate:1100 },
  { typ:'Lastväxlarflak',       veh:'KEM-02', tim:3.0, rate:1029 },
  { typ:'Stål och armering',    veh:'KEM-05', tim:3.5, rate:1029 },
  { typ:'Rivningsmassor',       veh:'KEM-04', tim:4.0, rate:767  },
];

// ── Quote price builder — 2026 verified Stockholm rates ───────────────────────
// Framkörning: 1.5h for crane/trailer, 1.0h for others (Svelands verified 2026)
// Minsta debitering: 3h crane, 2h others
// Per-mil: solobil 255 kr/mil (25.5 kr/km), med släp 315 kr/mil (31.5 kr/km)
//   — applied to routes >80 km; shorter routes use local handling contribution
// DMT (drivmedelstillägg): fuel cost passed through at 12% handling margin
function calcPrice(route, cargo, idx) {
  const isCrane    = cargo.veh === 'KEM-01';
  const hasTrailer = cargo.veh === 'KEM-05';

  const framkornH = (isCrane || hasTrailer) ? 1.5 : 1.0;
  const minBillH  = isCrane ? 3.0 : 2.0;
  const workH     = Math.max(cargo.tim + (idx % 3) * 0.5, minBillH);
  const billableH = workH + framkornH;

  // Time cost at verified 2026 rate
  const arbKost = Math.round(billableH * cargo.rate);

  // Fuel cost (internal bränsle_kostnad)
  const fuel = Math.round(route.km * VEHICLE_FUEL[cargo.veh] * DIESEL_PRICE);

  // DMT charged to customer (fuel passthrough + 12% handling)
  const dmt = Math.round(fuel * 1.12);

  // Distance component
  const kmRate = hasTrailer ? 31.5 : 25.5;
  const distKost = route.km >= 80
    ? Math.round(route.km * kmRate)                       // per-mil for long-haul
    : route.km >= 40
      ? Math.round(route.km * kmRate * 0.55)              // partial for medium routes
      : Math.round(route.km * 9.5);                       // local congestion/wear

  return {
    price:    arbKost + distKost + dmt,
    fuel,
    dmt,
    arbKost,
    tim:      billableH,
    conf:     Math.min(0.97, 0.82 + (idx % 10) * 0.015),
  };
}
...
export function seedDemoData() {
  ...
  db.transaction(() => {
    ...
    for (const v of FLEET) {
      ...
      const id = insFleet.run(
        CID, v.ext_id, v.reg, v.namn, v.lasttyp, v.typ, v.max_last_kg,
        v.volym_m3, v.lez_godkand, v.euro_klass, v.timkostnad_sek, v.priskm_sek,
        v.startavgift_sek, v.forbrukning_l_per_km, v.beskrivning
      ).lastInsertRowid;
    ...
    // ── 4. Quotes — 60 recent (1–89 days) + 23 older (91–178 days) ────────────
    ...
    // Status distribution: 22 godkänd, 14 skickad, 11 avböjd, 8 väntande, 5 motbud
    ...
    const quoteIds = [];
    const godkandQuoteIds = [];
    const motbudQuoteIds  = [];

    for (let i = 0; i < 60; i++) {
      const status   = recentStatuses[i];
      const daysBack = Math.round(1 + (i / 59) * 88);
      const route    = pick(ROUTES, i * 3 + 7);
      const cargo    = pick(CARGO,  i * 5 + 2);
      const cust     = CUSTOMERS[i % CUSTOMERS.length];

      const { price, fuel, dmt, arbKost, tim, conf } = calcPrice(route, cargo, i);
      ...
      track('quotes', qId);
      quoteIds.push(qId);
      if (status === 'godkänd') godkandQuoteIds.push({ qId, daysBack, route, cargo, price, dmt, cust, i });
      if (status === 'motbud')  motbudQuoteIds.push(qId);
    }

    // 23 older accepted quotes (91–178 days ago) to bring jobs to 45
    for (let i = 0; i < 23; i++) {
      ...
      const { price, fuel, dmt, arbKost, tim } = calcPrice(route, cargo, i + 60);
      ...
      track('quotes', qId);
      quoteIds.push(qId);
      godkandQuoteIds.push({ qId, daysBack, route, cargo, price, dmt, cust, i: i + 60 });
    }
    ...
    // Distribution: 18 slutförd, 8 fakturerad, 6 pågående, 13 planerad
    const JOB_STATUSES = [
    ...
    for (let i = 0; i < 45; i++) {
      const { qId, daysBack, price, dmt, cust } = godkandQuoteIds[i];
      ...
      track('jobs', jobId);
      jobRecords.push({ jobId, status, quoteId: qId, daysBack, price, dmt, custId, cust });
    }

    // ── 6. Invoices — 26 total ─────────────────────────────────────────────────
    // slutförd jobs → 18 betald
    // fakturerad jobs → 2 betald, 4 utestaende, 2 förfallen
    ...
    const fakturaStatus = ['betald','betald','utestaende','utestaende','utestaende','utestaende','förfallen','förfallen'];

    const toInvoice = [
      ...slutfordJobs.map(j => ({ ...j, invStatus: 'betald' })),
      ...fakturJobs.map((j, fi) => ({ ...j, invStatus: fakturaStatus[fi] })),
    ];

    for (const { jobId, price, dmt, cust, daysBack, invStatus } of toInvoice) {
      fCounter++;
      const fnr        = `FAK-2026-${String(fCounter).padStart(3, '0')}`;
      const dmtAmt     = dmt ?? 0;
      const transport  = price - dmtAmt;
      ...
      const lineItems  = JSON.stringify([
        { beskrivning: 'Transporttjänst',            antal: 1, apris: transport, belopp: transport },
        { beskrivning: 'Drivmedelstillägg (DMT)',    antal: 1, apris: dmtAmt,    belopp: dmtAmt    },
      ]);
    ...
    for (const m of MAINT_CONFIG) {
      ...
      insMaint.run(
        CID, m.ext_id, m.bes.slice(0,10), m.ins.slice(0,10), m.adr?.slice(0,10) ?? null,
    ...
    const COST_RECORDS = [
      ...
      ['KEM-04', 'service',    'Bromsservice + belägg',         11200, ago(180), 605000],
    ...
    const PARTNERS = [
      ...
      { name:'Mälaren Frakt AB',       org:'556567-8901', contact:'Björn Gustafsson', phone:'+46 70 334 45 56', email:'bjorn@malarenfrakt.se',        notes:'Mälardalen och Västerås-regionen.' },
      { name:'Roslagen Schakt AB',     org:'556678-9012', contact:'Gunnar Holm',      phone:'+46 70 445 56 67', email:'gunnar@roslagenschakt.se',     notes:'Schakt och massor i Norrtälje-regionen.' },
    ];
    const partnerIds = [];
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const litres = 70 + (i % 10) * 12;                      // 70–178 L
      const ppl    = 21.30 + (i % 6) * 0.08;                 // 21.30–21.78 kr/L, avg ≈ 21.54
