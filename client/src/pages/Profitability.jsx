  ...
  return (
    <Card padding="22px 24px 18px" style={{ flex: 1 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
        <span style={{ fontFamily: INTER, fontSize: 26, fontWeight: 700, color: TEXT, lineHeight: 1, fontFeatureSettings: '"tnum"' }}>
          {value ?? '—'}
        </span>
        {unit && value != null && (
          <span style={{ fontFamily: INTER, fontSize: 15, color: MUTED, fontFeatureSettings: '"tnum"' }}>{unit}</span>
        )}
      </div>
      {sublabel && (
            ...
            {rows.map((j, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td style={{ fontFamily: INTER, fontSize: 13, padding: '12px 16px', textAlign: 'right', color: TEXT, fontWeight: 600, fontFeatureSettings: '"tnum"' }}>{fmtSEK(j.intakt)}</td>
                  <td style={{ fontFamily: INTER, fontSize: 13, padding: '12px 16px', textAlign: 'right', color: MUTED, fontFeatureSettings: '"tnum"' }}>{fmtSEK(j.kostnad)}</td>
                  <td style={{ fontFamily: OUTFIT, fontSize: 13, padding: '12px 16px', textAlign: 'right' }}>
                    {j.marginal_pct != null ? (
                      <span style={{
                        display: 'inline-block', padding: '3px 10px', borderRadius: 6,
                        fontSize: 12, fontWeight: 600, fontFamily: INTER, fontFeatureSettings: '"tnum"',
