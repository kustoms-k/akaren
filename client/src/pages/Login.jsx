...
// ── Feature icons ─────────────────────────────────────────────────────────────
function IconAI() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6.5" stroke="rgba(94,234,212,0.5)" strokeWidth="1.2"/><path d="M5 8.5L7 10.5L11 6" stroke={CYAN} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>;
}
function IconLink() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M6 10L10 6M7 5.5H4.5a2 2 0 000 4h1M9 10.5h2.5a2 2 0 000-4H9" stroke={CYAN} strokeWidth="1.4" strokeLinecap="round"/></svg>;
}
function IconChart() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="3" y="9" width="2.5" height="4" rx="1" fill={CYAN} opacity="0.7"/><rect x="6.75" y="6" width="2.5" height="7" rx="1" fill={CYAN}/><rect x="10.5" y="3" width="2.5" height="10" rx="1" fill={CYAN} opacity="0.7"/></svg>;
}
function IconShield() {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ marginBottom: 36 }}><LogoMark size={60} glow /></div>
        <div style={{
        ...
        <div style={{
          ...
          letterSpacing: '0.22em', color: TEXT_MU, marginBottom: 28, textTransform: 'uppercase',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {features.map((f, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span style={{ fontFamily: INTER, fontSize: 13, color: TEXT_SEC, fontWeight: 400, lineHeight: 1.45 }}>
                {f.text}
              </span>
            </div>
          ))}
        </div>
        <div style={{ marginTop: 52, fontFamily: INTER, fontSize: 10, color: TEXT_MU, letterSpacing: '0.05em' }}>
...
function BankIDCard({ qrCode, hintCode, autoStartUrl, onCancel, simulated }) {
  ...
  return (
    <div style={{
      background: 'rgba(20,27,45,0.8)', border: '1px solid rgba(25,62,143,0.4)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {simulated && (
          <span style={{
            fontFamily: MONO, fontSize: 9, color: CYAN,
      ...
      {qrCode && (
        <div style={{
          ...
          boxShadow: '0 0 24px rgba(25,62,143,0.25)',
        }}>
      ...
      <button
        ...
        style={{
          ...
          color: TEXT_MU, background: 'none', border: 'none', cursor: 'pointer', padding: 0,
        }}
      >
        Avbryt
      </button>
...
export function Login() {
  ...
  // ── BankID state ──────────────────────────────────────────────────────────
  // loginMode: 'bankid-idle' | 'bankid-qr' | 'bankid-success' | 'bankid-error' | 'email'
  ...
  // ── Stop polling on unmount or mode change ────────────────────────────────
  function stopPoll() {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (data.status === 'complete') {
          stopPoll();
          setLoginMode('bankid-success');
          setTimeout(() => login(data), 600);
        } else if (data.status === 'no_user') {
          stopPoll();
          setBankidError('Ingen behörighet — kontakta din administratör.');
          setLoginMode('bankid-error');
        } else if (data.status === 'failed') {
          stopPoll();
          setBankidError(hintMsg(data.hintCode));
          setLoginMode('bankid-error');
        }
  ...
  async function startBankID() {
    ...
    try {
      ...
      setBankidOrderRef(data.orderRef);
      setBankidQR(data.qrCode);
      setBankidAutoUrl(data.autoStartUrl);
      setBankidHint(null);
      setBankidSimulated(data.simulated ?? false);
      setLoginMode('bankid-qr');
    } catch (err) {
      setBankidError(err.message || 'BankID-tjänsten är inte tillgänglig.');
      setLoginMode('bankid-error');
    } finally {
    ...
    return (
      <button
        ...
        style={{
          ...
          background: 'transparent', color: isActive ? CYAN : TEXT_MU,
        ...
        <div style={{
          ...
          <div style={{ width: '100%', maxWidth: 360 }}>
                ...
                {['en', 'sv'].map((l) => (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer', transition: 'all 0.15s cubic-bezier(0.22,1,0.36,1)', letterSpacing: '0.06em',
                    }}
                  >
                    {l.toUpperCase()}
                  </button>
            ...
            <div style={{ marginBottom: 28 }}>
              <div style={{ fontFamily: INTER, fontWeight: 700, fontSize: 26, color: TEXT_PR, marginBottom: 6, letterSpacing: '-0.02em', lineHeight: 1.2 }}>
            ...
            {tab === 'login' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <>
                    <button
                      ...
                      style={{
                        ...
                        background: bankidLoading ? 'rgba(25,62,143,0.5)' : BANKID_BLUE,
                        color: '#fff', cursor: bankidLoading ? 'not-allowed' : 'pointer',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
                        boxShadow: bankidLoading ? 'none' : '0 0 24px rgba(25,62,143,0.5)',
                        transition: 'transform 0.15s, box-shadow 0.15s',
                        letterSpacing: '0.02em',
                      }}
                      onMouseEnter={(e) => { if (!bankidLoading) { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 4px 30px rgba(25,62,143,0.7)'; } }}
                      onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = bankidLoading ? 'none' : '0 0 24px rgba(25,62,143,0.5)'; }}
                    >
                    ...
                    <button
                      ...
                      onMouseEnter={(e) => { e.currentTarget.style.borderColor = CYAN; e.currentTarget.style.background = 'rgba(94,234,212,0.05)'; }}
                    ...
                    <button
                      ...
                      style={{
                        ...
                        textDecoration: 'underline', padding: '4px 0', textDecorationColor: 'rgba(90,100,120,0.4)',
                      }}
                    >
                ...
                {/* BankID success animation */}
                {loginMode === 'bankid-success' && (
                  <div style={{
                    background: 'rgba(30,120,80,0.12)', border: '1px solid rgba(30,120,80,0.35)',
                    borderRadius: 16, padding: '32px 24px',
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12,
                  }}>
                    <div style={{ fontSize: 36 }}>✓</div>
                    <p style={{ fontFamily: INTER, fontSize: 14, fontWeight: 600, color: '#4ade80', margin: 0 }}>
                      Inloggning lyckades
                    </p>
                  </div>
                )}
                  ...
                  <>
                    <div style={{
                      ...
                      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ flexShrink: 0, marginTop: 1 }}>
                        <circle cx="8" cy="8" r="6.5" stroke={DANGER} strokeWidth="1.3"/>
                        <path d="M8 5v3.5M8 10.5v.5" stroke={DANGER} strokeWidth="1.5" strokeLinecap="round"/>
                      </svg>
                      <span style={{ fontFamily: INTER, fontSize: 13, color: DANGER, lineHeight: 1.5 }}>{bankidError}</span>
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: 'rgba(255,255,255,0.06)', color: TEXT_SEC, border: 'none',
                          borderRadius: 9, padding: '10px', cursor: 'pointer',
                        }}
                      >
                        Logga in med e-post
                ...
                {loginMode === 'email' && (
                  <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    {loginError && (
                      <div style={{
                        background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)',
                        borderRadius: 10, padding: '10px 14px', color: DANGER, fontSize: 13, fontFamily: INTER,
                    ...
                    <button
                      ...
                      style={{
                        ...
                        padding: '13px', borderRadius: 10, border: 'none', marginTop: 2,
                        letterSpacing: '0.04em',
                        background: canLogin ? `linear-gradient(135deg, ${CYAN_BR} 0%, ${CYAN} 100%)` : 'rgba(255,255,255,0.06)',
                    ...
                    <button
                      ...
                      style={{
                        fontFamily: INTER, width: '100%', fontWeight: 500, fontSize: 13,
                        padding: '11px', borderRadius: 10, border: '1px solid rgba(94,234,212,0.25)',
                        background: 'transparent', color: CYAN,
                    ...
                    <button
                      type="button"
                      onClick={() => setLoginMode('bankid-idle')}
                      style={{
              ...
              <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: canReg ? `linear-gradient(135deg, ${CYAN_BR} 0%, ${CYAN} 100%)` : 'rgba(255,255,255,0.06)',
